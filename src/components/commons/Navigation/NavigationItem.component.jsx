        ...
        {() => (
          <>
            ...
            <Transition
              ...
              leaveFrom='opacity-100 translate-y-0'
              leaveTo='opacity-0 translate-y-1'>
              <Popover.Panel static className='sub-menu will-change-transform absolute transform z-10 w-60 pt-3 left-0'>
                <ul className='rounded-xl shadow-lg ring-1 ring-black/5 dark:ring-white/10 text-sm relative bg-white dark:bg-neutral-800 py-4 grid space-y-1'>
                  {menuDropdown.children?.map((i) => {
                    if (i.type) return renderDropdownMenuNavlinkHasChild(i);
                    else {
                      return (
  ...
  const renderDropdownMenuNavlink = (item) =>
    item.targetBlank ? (
      <a
        ...
        {item.isNew && (
          <span className='bg-red-500 text-white text-[10px] px-1.5 py-1 leading-none rounded-md ml-2'>New!</span>
        )}
      </a>
    ) : (
      <NavLink
