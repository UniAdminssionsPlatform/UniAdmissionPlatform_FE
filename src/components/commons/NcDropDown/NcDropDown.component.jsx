  ...
  <Menu as='div' className='relative inline-block text-left'>
    <Menu.Button className={className} title={title}>
      {renderTrigger ? (
        ...
        <DotsHorizontalIcon className={iconClass} aria-hidden='true' stroke='none' fill='currentColor' />
      )}
    </Menu.Button>
    <Transition
      as={Fragment}
            ...
            <Menu.Item key={item.id} onClick={() => onClick(item)} data-menu-item-id={item.id}>
              {() =>
                renderItem && typeof renderItem(item) !== 'undefined' ? (
                  ...
                  <button
                    className={`flex items-center rounded-md w-full px-3 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:text-neutral-900 dark:hover:text-neutral-100 truncate ${twFocusClass()}`}>
